...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # for example, the A0 level will iterate through all possible actions for the problem and 
        # add a PgNode_a to a_levels[0]
        #   set iff all prerequisite literals for the action hold in S0.  This can be accomplished by testing
        #   to see if a proposed PgNode_a has prenodes that are a subset of the previous S level.  Once an
        #   action node is added, it MUST be connected to the S node instances in the appropriate s_level set.        
        
        # def action_satisfies_node(action, node_s):            
        #     if node_s.is_pos:
        #         for precond_pos in action.precond_pos:
        #             if precond_pos == node_s.symbol:
        #                 return True 

        #         return False 
        #     else:
        #         for precond_neg in action.precond_neg:
        #             if precond_neg == node_s.symbol:
        #                 return False
        ...
        #         if action_satisfies_node(action, node_s):
        #             # create new action node 
        #             node_a = PgNode_a(action)
        #             # connection action with state 
        #             node_a.parents.add(node_s)
        #             node_s.children.add(node_a)
        #             # add action to actions for this level 
    ...
    def add_literal_level(self, level):
        ...
        level_action_nodes = self.a_levels[level-1]

        for a_node in level_action_nodes:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return len([eff for eff in node_a1.action.effect_add if eff in node_a2.action.effect_rem]) > 0 or  len([eff for eff in node_a1.action.effect_rem if eff in node_a2.action.effect_add]) > 0
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        violated_a1_precondition = len([eff for eff in node_a1.action.precond_pos if eff in node_a2.action.effect_rem]) > 0 or  len([eff for eff in node_a1.action.precond_neg if eff in node_a2.action.effect_add]) > 0
        violated_a2_precondition = len([eff for eff in node_a2.action.precond_pos if eff in node_a1.action.effect_rem]) > 0 or  len([eff for eff in node_a2.action.precond_neg if eff in node_a1.action.effect_add]) > 0

        return violated_a1_precondition or violated_a2_precondition
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #mutual_exclusion = len([eff for eff in node_a1.action.precond_pos if eff in node_a2.action.precond_neg]) > 0 or len([eff for eff in node_a1.action.precond_neg if eff in node_a2.action.precond_pos]) > 0
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # You need to verify that if there is a parents pair (p1, p2) that not p1 
        # is mutex of p2, and not p2 is mutex of p1 the inconsistent support mutex is False.

        # TODO test for Inconsistent Support between nodes
        from itertools import product
        mutex = any(1 for p1,p2 in product(node_s1.parents, node_s2.parents)
    ...
    def h_levelsum(self) -> int:        
        ...
        for goal in self.problem.goal:
            for i, s_node_set in enumerate(self.s_levels): 
                for s_node in s_node_set:                    
                    literal = expr(s_node.symbol)
                    if s_node.is_pos and goal == literal:
                        if s_node not in state_count:
                            state_count[s_node] = i 
                        elif state_count[s_node] > i:
                            state_count[s_node] = i                        
                        break 
